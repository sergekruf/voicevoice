        ...
        Form {
            Section("Модель распознавания") {
                Picker(selection: $settings.sttEngineRaw) {
                    ...
                    HStack(spacing: 4) {
                        Text("Движок")
                        HelpHint(text: "Какой движок распознавания использовать. Parakeet TDT v3 (по умолчанию) — быстрый движок NVIDIA через FluidAudio (CoreML/ANE): ~5× быстрее и меньше памяти, чем Whisper, хорошо распознаёт русский. Знаки препинания расставляет слабее, но это компенсируется пост-обработкой. Модель ~600 МБ скачивается при первом запуске. WhisperKit (Whisper) — классический движок: чуть лучше с пунктуацией и шире поддержка языков; переключитесь на него, если диктуете не на русском или важна пунктуация без пост-обработки.")
                    }
                }
            ...
            Section("Активация") {
                ...
                Toggle(isOn: $settings.showResultHUD) {
                    HStack(spacing: 4) {
                        ...
                        HelpHint(text: "Маленькое окошко в углу экрана с результатом распознавания. Если выключено — текст вставляется молча, без визуального подтверждения. Учтите: при включённом «тихом режиме» эта опция игнорируется и HUD всё равно не показывается.")
                    }
                }
